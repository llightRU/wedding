 ...
 * MUST be used client-side only (requires document).
 */
...
export async function postToGoogleSheets(data: RsvpPayload): Promise<{ success: boolean; message: string }> {
    if (!APPS_SCRIPT_URL) {
    ...
    return new Promise((resolve) => {
        try {
            ...
            const locationMap: Record<string, string> = { groom: 'Nhà trai', bride: 'Nhà gái', both: 'Cả hai' };
